...
function crmDealToOrder(deal) {
  const description = deal.Description || '';
  const conNote = descriptionField(description, 'Con note') || deal.Deal_Name?.split(' - ').at(-1) || deal.id;
  const vendor = descriptionField(description, 'Supplier') || 'Supplier';
  const dropLocation = descriptionField(description, 'Delivery address') || '';
  const priority = descriptionField(description, 'Priority') || 'next-run';
  ...
  return {
    ...
    urgency: priority === 'asap' ? 'asap' : 'next-run',
...
function mergeById(primary = [], secondary = []) {
  ...
  for (const item of primary) byId.set(item.id, { ...byId.get(item.id), ...item });
  return [...byId.values()];
}

async function zohoCRMDealsForWorkspace({ role, email } = {}) {
  const token = await accessTokenFor('CRM');
  if (!token) return [];

  let client = null;
  if (role === 'client' && email) {
    client = await zohoCRMClientByEmail(email);
    if (!client) return [];
  }
  ...
  const allowedStages = dealStages();

  return allDeals
    .filter(deal => !deal.Pipeline || deal.Pipeline === pipeline || deal.Pipeline?.display_value === pipeline || allowedStages.has(deal.Stage))
    .filter(deal => {
      if (role !== 'client') return true;
      return (
        deal.Contact_Name?.id === client?.zohoContactId ||
        deal.Account_Name?.id === client?.zohoAccountId ||
        deal.Contact_Name?.name === client?.name ||
        deal.Account_Name?.name === client?.businessName
      );
    })
    .map(crmDealToOrder);
}

async function workspaceForViewer({ role, email } = {}) {
  let orders = memoryStore.orders || [];
  if (email && ['client', 'admin', 'driver'].includes(role || '')) {
    try {
      const zohoOrders = await zohoCRMDealsForWorkspace({ role, email });
      orders = mergeById(zohoOrders, orders);
    } catch (error) {
      console.error('Could not pull Zoho Deals:', error);
    }
  }
...
export async function handler(event) {
  ...
  try {
    ...
    if (event.httpMethod === 'GET' && path === '/workspace') {
      return response(200, { store: await workspaceForViewer(sessionFromEvent(event) || {}) });
    }
    ...
    if (event.httpMethod === 'POST' && path === '/auth/login') {
      ...
      return response(200, { user: publicUser(user) }, sessionCookieFor(user));
    }
    ...
    if (event.httpMethod === 'POST' && path === '/auth/verify-code') {
      ...
      return response(result.success ? 200 : 401, result, result.success ? sessionCookieFor(result.user) : {});
    }
