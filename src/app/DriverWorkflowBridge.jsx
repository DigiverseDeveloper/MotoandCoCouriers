...
function headingFor(tab) {
  if (tab === "pickup") return ["Pickup", "Confirmation"];
  if (tab === "delivery") return ["Delivery", "Confirmation"];
  return ["Milk", "Run"];
}
...
export default function DriverWorkflowBridge() {
  ...
  const routeOrders = orders.filter(order => ["Order Placed", "Picked Up", "In Transit"].includes(normaliseStatus(order.status)));
  ...
  const routeGroups = useMemo(() => groupBy(routeOrders, order => order.vendor || "Unknown vendor"), [routeOrders]);
  ...
  const [headingLead, headingTail] = headingFor(tab);
    ...
    <div className="dw-shell">
      ...
      <header className="dw-topbar">
        ...
        <nav className="dw-nav">
          <button className={tab === "route" ? "active" : ""} onClick={() => setTab("route")}>Milk Run</button>
          <button className={tab === "pickup" ? "active" : ""} onClick={() => setTab("pickup")}>Pickup Confirmation</button>
          <button className={tab === "delivery" ? "active" : ""} onClick={() => setTab("delivery")}>Delivery Confirmation</button>
        </nav>
      ...
      <main className="dw-page">
        <section className="dw-titlebar">
          <div>
            <h1><span>{headingLead}</span> {headingTail}</h1>
            <p>{today} - {driver.name || "Driver"} - Brisbane to Gold Coast</p>
          </div>
          <div className="dw-stats">
            <div><strong>{routeOrders.length}</strong><span>Run</span></div>
            <div><strong>{pickupOrders.length}</strong><span>Pickup</span></div>
            <div><strong>{deliveryOrders.length}</strong><span>Delivery</span></div>
        ...
        {store && tab === "route" && (
          <>
            <div className="dw-alert soft">Use this screen to plan the run before confirming pickups. It shows each supplier stop and the customer drop addresses attached to that stop.</div>
            {Object.keys(routeGroups).length === 0 && <div className="dw-empty">No active milk run stops right now.</div>}
            {Object.entries(routeGroups).map(([vendor, groupOrders], index) => {
              const firstOrder = groupOrders[0] || {};
              return (
                <section className="dw-vendor" key={vendor}>
                  <h2>Stop {index + 1} - {vendor}</h2>
                  <div className="dw-route-card">
                    <div className="dw-stop-head legacy-head">
                      ...
                      <span>{groupOrders.length} customer{groupOrders.length === 1 ? "" : "s"}</span>
                    </div>
                    <div className="dw-route-list">
                      {groupOrders.map(order => (
                        <div className="dw-route-row" key={order.id}>
                          <strong>{order.businessName || order.clientName || "Customer"}</strong>
                          <span>{order.dropLocation || "No delivery address set"}</span>
                          <em>{order.conNote || "No con note"} - {normaliseStatus(order.status)}</em>
                        </div>
                      ))}
                    </div>
                  </div>
